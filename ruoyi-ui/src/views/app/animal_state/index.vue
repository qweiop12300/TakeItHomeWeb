    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['app:animal_state:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['app:animal_state:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['app:animal_state:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['app:animal_state:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="animal_stateList" @selection-change="handleSelectionChange">
...
<script>
import { listAnimal_state, getAnimal_state, delAnimal_state, addAnimal_state, updateAnimal_state } from "@/api/app/animal_state";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAnimal_state(this.queryParams).then(response => {
        this.animal_stateList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAnimal_state(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateAnimal_state(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAnimal_state(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除动物状态关联编号为"' + ids + '"的数据项？').then(function() {
        return delAnimal_state(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('app/animal_state/export', {
        ...this.queryParams
      }, `animal_state_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
