<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="话题名字" prop="title">
        <el-input
    ...
    <el-table v-loading="loading" :data="topicList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="话题名字" align="center" prop="title" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="话题名字" prop="title">
          <el-input v-model="form.title" placeholder="请输入话题名字" />
        </el-form-item>
      </el-form>
...
<script>
import { listTopic, getTopic, delTopic, addTopic, updateTopic } from "@/api/app/topic";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除话题编号为"' + ids + '"的数据项？').then(function() {
        return delTopic(ids);
      }).then(() => {
        this.getList();
